...
public class NanotechTeleporter extends Teleporter
{
            ...
            for(int l = -2; l <= 2; ++l)
            {
                for(int i1 = -2; i1 <= 2; ++i1)
                {
                    for(int j1 = -1; j1 < 3; ++j1)
                    {
                        ...
                        this.worldServerInstance.setBlock(k1, l1, i2, flag ? NanotechBlock.portalFrame : Blocks.air);
                    }
                }
            }
    ...
    public boolean placeInExistingPortal(Entity entity, double x, double y, double z, float rotationYaw)
    {
        ...
        else
        {
            for(k1 = l - short1; k1 <= l + short1; ++k1)
            {
                ...
                for(int l1 = i1 - short1; l1 <= i1 + short1; ++l1)
                {
                    ...
                    {
                        if(this.worldServerInstance.getBlock(k1, i2, l1) == NanotechBlock.nanoPortal)
                        {
                            while(this.worldServerInstance.getBlock(k1, i2 - 1, l1) == NanotechBlock.nanoPortal)
                            {
                                --i2;
                            }
        ...
        if(d3 >= 0.0D)
        {
            ...
            d4 = (double)k + 0.5D;
            int j2 = -1;
            if(this.worldServerInstance.getBlock(i - 1, j, k) == NanotechBlock.nanoPortal)
            {
                j2 = 2;
            }
            if(this.worldServerInstance.getBlock(i + 1, j, k) == NanotechBlock.nanoPortal)
            ...
            if(this.worldServerInstance.getBlock(i, j, k - 1) == NanotechBlock.nanoPortal)
            {
                j2 = 3;
            }
            if(this.worldServerInstance.getBlock(i, j, k + 1) == NanotechBlock.nanoPortal)
            ...
            for(k2 = -1; k2 <= 1; ++k2)
            {
                for(i3 = 1; i3 < 3; ++i3)
                {
                    for(l2 = -1; l2 < 3; ++l2)
                    {
                        ...
                        this.worldServerInstance.setBlock(k3, j3, i4, flag ? NanotechBlock.portalFrame : Blocks.air);
                    }
                }
            }
        }
        for(k2 = 0; k2 < 4; ++k2)
        {
            for(i3 = 0; i3 < 4; ++i3)
            {
                for(l2 = -1; l2 < 4; ++l2)
                {
                    ...
                    this.worldServerInstance.setBlock(k3, j3, i4, flag ? NanotechBlock.portalFrame : NanotechBlock.nanoPortal, 0, 2);
                }
            }
            for(i3 = 0; i3 < 4; ++i3)
            {
