            ...
            if(canOpen)
            {
                for(int j = -1; j < 2; ++j)
                {
                    for(int k = -1; k < 2; ++k)
                    {
                        if(j == 0 && k == 0)
                        {
                            this.worldObj.setBlock(xCoord, yCoord + 2, zCoord, NTMBlocks.sunShade, 3, 3);
                        }
                        else
                        {
                            this.worldObj.setBlock(xCoord + j, yCoord + 2, zCoord + k, NTMBlocks.sunShade, 2, 3);
                        }
                    }
        ...
        if(this.worldObj.getBlockMetadata(xCoord, yCoord + 2, zCoord) == 3)
        {
            for(int j = -1; j < 2; ++j)
            {
                for(int k = -1; k < 2; ++k)
                {
                    if(j == 0 && k == 0)
                    {
                        this.worldObj.setBlock(xCoord, yCoord + 2, zCoord, NTMBlocks.sunShade, 1, 3);
                    }
                    else
                    {
